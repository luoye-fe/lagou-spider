...
getAllCitysArr()
		...
		function loop() {
			...
			if (!city) {
				let mailOptions = {
					...
					html: `
	本次爬取开始时间：${new Date(beginTime).toLocaleTimeString('ja-chinese', {year: "numeric", month: 'numeric', day: 'numeric'})}<br>
	...
	`
				...
				sendMail(mailOptions, (err) => {
					console.log(
						`
	本次爬取开始时间：${new Date(beginTime).toLocaleTimeString('ja-chinese', {year: "numeric", month: 'numeric', day: 'numeric'})}
